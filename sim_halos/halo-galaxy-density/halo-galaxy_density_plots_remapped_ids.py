...
class HaloDensityAnalyzer:
    ...
    def load_data(self, halo_file_path, galaxy_file_path, galaxy_remap_file_path):
        """
        ...
        print(f"Loading galaxy remap file: {galaxy_remap_file_path}")
        remap_df = pd.read_csv(galaxy_remap_file_path)
        print(f"Loaded {len(remap_df)} remapped galaxy IDs")
        ...
        # Remap galaxy group IDs
        galaxy_df = galaxy_df.merge(remap_df, left_on='id_galaxy_sky', right_on='id_galaxy_sky', how='left')
        galaxy_df['id_group_sky'] = galaxy_df['new_group_id'].fillna(galaxy_df['id_group_sky'])
    ...
    def plot_histograms(self):
        ...
        plt.suptitle(
            f'Halo–Galaxy Pairs (Remapped Group IDs, log10(mvir) > {self.min_log_halo_mass})',  # Removed reference to undefined variable
            fontsize=16, fontweight='bold'
        )
        plt.tight_layout()
        plt.savefig('herons_filters_remapped.png', dpi=300)
        plt.show()
    
    def run_analysis(self, halo_file_path, galaxy_file_path, remapped_gal_ids_path, output_path='halo_density_plots.png'):
        """
        ...
        self.load_data(halo_file_path, galaxy_file_path, remapped_gal_ids_path)
        self.make_kde_trees()
...
if __name__ == "__main__":
    ...
    remapped_gal_ids_path = '/Users/sp624AA/Code/waves_work/sim_halos/remap_sharks/galaxy_group_remap.csv'
    # Run analysis

    analyzer.run_analysis(halo_file_path, galaxy_file_path, remapped_gal_ids_path)
