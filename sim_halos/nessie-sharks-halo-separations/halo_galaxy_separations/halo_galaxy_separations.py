...
class SharksCatalogHaloHaloSeparations:
    ...
    def process_data(self):
        ...
        self.sharks['co_dist'] = get_all_comoving_distance(self.sharks['zobs'])
    ...
    def make_kde_trees(self):
        ...
        sharks_coords = find_all_spherical_to_cartesian(self.sharks['ra'], self.sharks['dec'], self.sharks['co_dist'])
        
        self.tree_nessie = KDTree(cartesian_nessie)
        self.tree_herons = KDTree(cartesian_herons)
        self.tree_sharks = KDTree(sharks_coords)
    ...
    def populate_2d_histograms(self):
        ...
        # Nessie
        self.h_nessie_tot = np.zeros((proj_bins, vel_bins))
        self.h_nessie_isolated = np.zeros((proj_bins, vel_bins))
        self.h_nessie_in_group = np.zeros((proj_bins, vel_bins))
        self.h_nessie_diff_group = np.zeros((proj_bins, vel_bins))

        n_groups_nessie = len(self.nessie_group_properties)

        print(f"Calculating nessie halo-galaxy pairs for {n_groups_nessie} groups...")
        ...
        for i in tqdm(range(n_groups_nessie)):
            ...
            distances, indices = self.tree_sharks.query(spherical_to_cartesian(ra1, dec1, co_dist1), k=self.k_nn)

            # Check max indexed galaxy is outside the max projected separation and velocity difference
            ra2 = self.sharks['ra'][indices[-1]]
            dec2 = self.sharks['dec'][indices[-1]]
            z2 = self.sharks['zobs'][indices[-1]]
            ...
            if proj_sep_norm >= self.max_proj_r50 or vel_diff_norm >= self.max_vel_sigma:

                for idx in indices:
                    ra2 = self.sharks['ra'][idx]
                    dec2 = self.sharks['dec'][idx]
                    z2 = self.sharks['zobs'][idx]
                    group_id2 = self.group_ids_nessie[idx]
                    ...
                    if proj_sep_norm <= self.max_proj_r50 and vel_diff_norm <= self.max_vel_sigma:
                        ...
                        if 0 <= proj_bin < proj_bins and 0 <= vel_bin < vel_bins:
                            self.h_nessie_tot[proj_bin, vel_bin] += 1
                            if group_id2 == -1:
                                self.h_nessie_isolated[proj_bin, vel_bin] += 1
                            elif group_id2 == self.nessie_group_properties['group_id'][i]:
                                self.h_nessie_in_group[proj_bin, vel_bin] += 1
                            else:
                                self.h_nessie_diff_group[proj_bin, vel_bin] += 1

            else:
                raise ValueError(f"Max indexed galaxy for group {i} is within the max cuts. Increase k in query.")
        ...
        print(f"Calculating herons halo-galaxy pairs for {n_groups_herons} groups...")
        for i in tqdm(range(n_groups_herons)):
            ...
            # Query the tree for neighbors within the max projected separation, getting the 250 nearest neighbors
            distances, indices = self.tree_sharks.query(spherical_to_cartesian(ra1, dec1, co_dist1), k=self.k_nn)
            ...
            if proj_sep_norm >= self.max_proj_r50 or vel_diff_norm >= self.max_vel_sigma:
                ...
                for idx in indices:
                    ...
                    dec2 = self.sharks['dec'][idx]
                    z2 = self.sharks['zobs'][idx]
                    group_id2 = self.group_ids_herons[idx]
                    # Calculate projected separation
                    proj_sep = find_projected_separation(ra1, dec1, ra2, dec2, co_dist1)
                    # Calculate velocity difference
                    vel_diff = calculate_velocity_difference(z1, z2)
                    ...
                    if proj_sep_norm <= self.max_proj_r50 and vel_diff_norm <= self.max_vel_sigma:
                        ...
                        if 0 <= proj_bin < proj_bins and 0 <= vel_bin < vel_bins:
                            ...
                            elif group_id2 == self.herons_group_properties['group_id'][i]:
    ...
    def plot_histograms(self):
        ...
        fig, axes = plt.subplots(4, 2, figsize=(16, 6*4))

        proj_bins = self.h_nessie_tot.shape[0]
        vel_bins = self.h_nessie_tot.shape[1]

        proj_bin_edges = np.linspace(0, self.max_proj_r50, proj_bins + 1)
        vel_bin_edges = np.linspace(0, self.max_vel_sigma, vel_bins + 1)

        # Nessie - All Galaxies
        im1 = axes[0, 0].imshow(self.h_nessie_tot.T, origin='lower', aspect='auto',
                        extent=[0, self.max_proj_r50, 0, self.max_vel_sigma], norm = 'log',
                        cmap='viridis')
        axes[0, 0].set_title(f'Nessie Groups- All Galaxies\n({int(np.sum(self.h_nessie_tot))} pairs - {len(self.nessie_group_properties)} groups)')
        plt.colorbar(im1, ax=axes[0, 0], label='Number of pairs')

        # Herons - All Galaxies
        im2 = axes[0, 1].imshow(self.h_herons_tot.T, origin='lower', aspect='auto',
                        extent=[0, self.max_proj_r50, 0, self.max_vel_sigma], norm = 'log',
                        cmap='viridis')
        axes[0, 1].set_title(f'Herons Groups- All Galaxies\n({int(np.sum(self.h_herons_tot))} pairs - {len(self.herons_group_properties)} groups)')
        plt.colorbar(im2, ax=axes[0, 1], label='Number of pairs')

        # Nessie Isolated
        im3 = axes[1, 0].imshow(self.h_nessie_isolated.T, origin='lower', aspect='auto',
                        extent=[0, self.max_proj_r50, 0, self.max_vel_sigma], norm = 'log',
                        cmap='viridis', )
        axes[1, 0].set_title(f'Nessie Groups- Isolated Galaxies\n({int(np.sum(self.h_nessie_isolated))} pairs)')
        plt.colorbar(im3, ax=axes[1, 0], label='Number of pairs')
        
        # Herons Isolated
        im4 = axes[1, 1].imshow(self.h_herons_isolated.T, origin='lower', aspect='auto',
                        extent=[0, self.max_proj_r50, 0, self.max_vel_sigma], norm = 'log',
                        cmap='viridis')
        axes[1, 1].set_title(f'Herons Groups- Isolated Galaxies\n({int(np.sum(self.h_herons_isolated))} pairs)')
        plt.colorbar(im4, ax=axes[1, 1], label='Number of pairs')
        
        # Nessie In Group
        im5 = axes[2, 0].imshow(self.h_nessie_in_group.T, origin='lower', aspect='auto',
                        extent=[0, self.max_proj_r50, 0, self.max_vel_sigma], norm = 'log',
                        cmap='viridis')
        axes[2, 0].set_title(f'Nessie Groups- In Group Galaxies\n({int(np.sum(self.h_nessie_in_group))} pairs)')
        plt.colorbar(im5, ax=axes[2, 0], label='Number of pairs')
        
        # Herons In Group
        im6 = axes[2, 1].imshow(self.h_herons_in_group.T, origin='lower', aspect='auto',
                        extent=[0, self.max_proj_r50, 0, self.max_vel_sigma], norm = 'log',
                        cmap='viridis')
        axes[2, 1].set_title(f'Herons Groups- In Group Galaxies\n({int(np.sum(self.h_herons_in_group))} pairs)')
        plt.colorbar(im6, ax=axes[2, 1], label='Number of pairs')
        
        # Nessie Different Group
        im7 = axes[3, 0].imshow(self.h_nessie_diff_group.T, origin='lower', aspect='auto',
                        extent=[0, self.max_proj_r50, 0, self.max_vel_sigma], norm = 'log',
                        cmap='viridis')
        axes[3, 0].set_title(f'Nessie Groups- Different Group Galaxies\n({int(np.sum(self.h_nessie_diff_group))} pairs)')
        plt.colorbar(im7, ax=axes[3, 0], label='Number of pairs')
        
        # Herons Different Group
        im8 = axes[3, 1].imshow(self.h_herons_diff_group.T, origin='lower', aspect='auto',
                        extent=[0, self.max_proj_r50, 0, self.max_vel_sigma], norm = 'log',
                        cmap='viridis')
        axes[3, 1].set_title(f'Herons Groups- Different Group Galaxies\n({int(np.sum(self.h_herons_diff_group))} pairs)')
        plt.colorbar(im8, ax=axes[3, 1], label='Number of pairs')

        # Formatting
        for i in range(4):
            for j in range(2):
                axes[i, j].set_xlabel('$rp / r50$')
                axes[i, j].set_ylabel('$c\Delta z/ \sigma_v$')
                axes[i, j].grid(True, alpha=0.3)

        # Set super title
        plt.suptitle(f'Sharks WAVESwide Halo-Galaxy Pairs Nessie vs Herons Group IDs, N >= {self.min_group_members}', fontsize=16, fontweight='bold', y=0.99)
        plt.tight_layout()
        plt.savefig('sharks_halo-galaxy_density_comparison.png', dpi=300)
        plt.show()
